...
static void update_timer(void)
{
        ...
        if(sec == 1) {
                fmt = "%d second remaining";
        } else {
                fmt = "%d seconds remaining";
        }

        snprintf(label, sizeof(label), fmt, sec);
...
static gboolean drop_event(GtkWidget *widget, GdkEvent *event, gpointer *data)
{
	(void)widget;
	(void)event;
	(void)data;
